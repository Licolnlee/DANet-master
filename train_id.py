...
from idcard_loader import IDCard
...
def train(args):
    ...
    train_data = IDCard(args.image_dir)
    ...
    # 从断点加载模型重新训练
    # model_path='experiments/checkpoint/unet/2022-10-20_2_0.0004_0_-1.0_1_idcard_train_rgb_50.pth'
    ...
    charb = Charbonnier()
    ssim = SSIM()
    df = pd.DataFrame(columns=['iter', 'loss1', 'loss2', 'total_loss'])
    ...
    for epoch in range(args.epochs):
        ...
        for img, gt in tqdm.tqdm(train_loader):
            ...
            if args.color == 'rgb':
                ...
                inner = torch.where(rs > 0)
                inner_b = b[inner]
                inner_gt = gt[inner]
                loss2 = 0.9 * charb(inner_b, inner_gt) + 0.1 * charb(b, gt) + ssim(b, gt)
            else:
        ...
        df = df.append(pd.Series(
            data=[epoch, total_loss1 / sample_train, total_loss2 / sample_train, total_loss / sample_train, val_loss],
